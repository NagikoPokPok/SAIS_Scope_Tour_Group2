...
class RabbitMQClient {
  ...
  async connect() {
    try {
      ...
      this.connection = await amqp.connect(RABBITMQ_URL, {
        servername: new URL(RABBITMQ_URL).hostname,
      });
  ...
  async recreateQueues() {
    ...
    for (const queue of Object.values(QUEUES)) {
      try {
        ...
        console.log(`Deleted queues for: ${queue}`);
      } catch (deleteError) {
        console.log(`Could not delete queue ${queue}: ${deleteError.message}`);
      }
    }
  ...
  async consumeFromQueue(queue, callback) {
    try {
      if (!this.isConnected || !this.channel) {
        console.log("RabbitMQ not connected, cannot consume from queue");
        return;
      }
          ...
          if (message) {
            try {
              ...
              if (result && result.success === true) {
                ...
                console.log(
                  `Message processed successfully and acknowledged from ${queue}`,
                );
              } else {
                ...
                if (result && result.reason === "database_disconnected") {
                  console.log(
                    "Database disconnected - message will be requeued",
                  );
                  this.channel.nack(message, false, true); // Requeue
                } else {
                  // Với lỗi khác, cũng requeue nhưng có thể limit retry
                  console.log("Processing failed - message will be requeued");
                  this.channel.nack(message, false, true); // Requeue
                }
              }
            } catch (error) {
              ...
              if (this.isDatabaseError(error)) {
                console.log(
                  "Database connection error detected - message will be requeued",
                );
                this.channel.nack(message, false, true); // Requeue
              } else {
                // Lỗi khác, gửi vào dead letter queue
                console.error(
                  "Non-retryable error, sending to dead letter queue",
                );
  ...
  async restartConsumers() {
    ...
    for (const [queue, callback] of this.consumers) {
      try {
        ...
        console.error(
          `Failed to restart consumer for queue ${queue}:`,
          error.message,
        );
      }
    }
  ...
  async sendToQueue(queue, message) {
    try {
      if (!this.isConnected || !this.channel) {
        console.log("RabbitMQ not connected, cannot send message");
        return false;
      }
  ...
  async publishToExchange(exchange, routingKey, message) {
    try {
      if (!this.isConnected || !this.channel) {
        console.log("RabbitMQ not connected, cannot publish message");
        return false;
      }
